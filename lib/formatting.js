...
export function formatFromIRCToDiscord(text, canPreviewURLs) {
  ...
  for (let i = 0; i <= blocks.length; i += 1) {
    ...
    // Add markers to prevent URL previews if applicable.
    if (!canPreviewURLs && block.text) {
        mdText += block.text.replace(urlRegex(), '<$&>');
    } else {
	mdText += block.text || '';
    }
  }
